...
La posibilidad de combinaciones es prácticamente infinita.
...
df2 <- df %>% 
  mutate(Nota_cat =  # <1>
           case_when(Nota < 5 ~ "Menos de 5",
                     ...
                     is.na(Nota) ~ "Sin nota"),
         Nota_cat = # <2>
           factor(Nota_cat, levels = c("Sin nota", "5 a más", "Menos de 5")))
...
ggplot(df, aes(x = Año)) + # <1>
  geom_histogram(fill = "steelblue") + # <2>
  facet_wrap(~ `Tipo filme`, scales = "free") +  # <3> Divide en facetas por grupo
...
```{r}
ggplot(df2, aes(y = Nota, x = `Tipo filme`)) + # <1>
  geom_boxplot() # <2>
```
